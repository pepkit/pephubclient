...
class PEPHubAuth(RequestManager):
    ...
    def login_to_pephub(self, base_url: str | None = None) -> str | None:
    ...
    def _request_pephub_for_device_code(self) -> InitializeDeviceCodeResponse:
        """
        Request a device code from pephub.
        """
    ...
    def _exchange_device_code_on_token(self, device_code: str) -> str:
        """
        Send a request with a device code to pephub in order to exchange it for a JWT.
    ...
    def _handle_pephub_response(
        ...
        """
        Decode the response from PEPhub and pack the returned data into the appropriate model.

        Args:
            response: Response from pephub.
            model: Model that the data will be packed into.

        Returns:
            Response data as an instance of the correct model.
        """

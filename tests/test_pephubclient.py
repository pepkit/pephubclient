...
@pytest.mark.parametrize(
    ...
    [
        (
            RegistryPath(namespace="geo", item="123"),
            {"DATA": "test"},
            "https://pephub.databio.org/pep/geo/123?DATA=test",
        ),
        (
            RegistryPath(namespace="geo", item="123"),
            {"DATA": "test", "VARIABLE": "value"},
            "https://pephub.databio.org/pep/geo/123?DATA=test&VARIABLE=value",
        ),
        (
            RegistryPath(namespace="geo", item="123"),
            {},
...
def test_request_pephub_creates_correct_url(
    registry_path, variables, expected_url, requests_get_mock
):
